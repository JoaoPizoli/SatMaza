    ...
    async listarClientes(repreId: string, busca?: string): Promise<any[]> {
        ...
        if (busca && busca.trim()) {
            return await this.erpDataSource.query(
                "SELECT CODCLI, NOMCLI, CODREP, NOMREP, CIDADE, UF FROM VW_CLIENTES_ATIVOS WHERE (NOMCLI LIKE ? OR CODCLI LIKE ?) AND CODREP = ?",
                [`%${busca}%`, `%${busca}%`, repreIdStr],
            );
        }

        return await this.erpDataSource.query(
            "SELECT CODCLI, NOMCLI, CODREP, NOMREP, CIDADE, UF FROM VW_CLIENTES_ATIVOS WHERE CODREP = ?",
            [repreIdStr],
        );
    }
